...
/**
 * 执行所有数据迁移
 * @param {Object} d - 原始数据对象
 * @param {Object} result - 包含 { categories, bookmarks, customAttributes, siblingGroups }
 * @returns {boolean} 是否需要持久化
 */
export function runMigrations(d, result) {
  ...
  } catch (e) { console.warn('[Migration] expand state error:', e.message) }
  ...
  result.siblingGroups.forEach(g => {
    ...
    if (!g.updatedAt) { g.updatedAt = Date.now(); g._migrated = true }
    if (g.useCount == null) { g.useCount = 0; g._migrated = true }
  })

  result.siblingGroups.forEach(g => { if (g._migrated) needsPersist = true })

  return needsPersist
}

function _migrateTextNotes(text, bookmarks, siblingGroups, group) {
  const ids = []
  ...
  while ((m = re2.exec(text)) !== null) {
    ...
    if (m[1] !== undefined) {
      const bm2 = bookmarks.find(x => x.id === m[2])
      if (bm2) html += _migInlineCardHTML(bm2)
      else html += esc(m[0])
    } else if (m[3] !== undefined) {
      const sg2 = siblingGroups.find(x => x.name === m[3])
...
function _migInlineCardHTML(bm) {
...
function _migGroupRefCardHTML(g) {
