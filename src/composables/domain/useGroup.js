...
export function saveGroupBody(gid) {
...
export function createGroup(catId) {
  const store = useAppStore();
...
export function deleteGroup(dGid, skipConfirm) {
  const store = useAppStore();
...
export function addToGroupDirect(bmId, tGid) {
  const store = useAppStore();
...
export function removeBmFromGroup(bmId, tGid) {
  const store = useAppStore();
  const sg = store.groupMap[tGid];
  if (!sg) return;
...
export function addGroupRefToGroup(refGid, targetGid, clientX, clientY) {
  const store = useAppStore();
...
export function removeGroupRef(targetGid, refGid) {
  const store = useAppStore();
  const ed = EditorManager.get(targetGid);
...
export function searchInFocusedGroup() {
  ...
  body.querySelectorAll('.group-inline-card').forEach(c => {
    ...
    if (bmId && bmId.startsWith('ref:')) {
      ...
      c.style.display = (rg && (rg.name || '').toLowerCase().indexOf(q) !== -1) ? '' : 'none';
    } else if (bmId) {
      const bm = store.bookmarkMap[bmId];
      c.style.display = (bm && (bm.title.toLowerCase().indexOf(q) !== -1 || bm.url.toLowerCase().indexOf(q) !== -1)) ? '' : 'none';
    }
  });
}

export function toggleGroupFocus(tGid) {
  const store = useAppStore();
...
export function editGroup(eGid) {
  const store = useAppStore();
...
export function removeFromSrcGroup(srcGid, bmId) {
