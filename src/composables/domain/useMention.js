...
export function useMention() {
  ...
  function showNear(query) {
    ...
    candidates.value = isGroup
      ? matches.map(g => ({ ...g, type: 'group' }))
      : matches.map(b => ({ ...b, type: 'bookmark', subItems: store.bookmarks.filter(s => s.parentId === b.id) || null }))
  ...
  function selectGroupRef(refGid) {
  ...
  function onTrigger(e) {
    ...
    if (!gb || !e.target.isContentEditable) return
    store.mentionGid = gb.closest('.group-card')?.dataset?.groupId || null
    store.mentionQuery = ''
  ...
  function onInput(e) {
    ...
    const gb = e.target.closest('.group-body')
    if (!gb || !e.target.isContentEditable || (gb.closest('.group-card')?.dataset?.groupId || null) !== store.mentionGid) { hide(); return }
    const sel = window.getSelection()
    if (!sel.rangeCount) { hide(); return }
    const node = sel.focusNode
    if (node.nodeType !== 3) { hide(); return }
    const text = node.textContent
    const trigger = store.mentionType === 'group' ? '#' : '@'
