...
function snapSize(s) { return (s.notes ? s.notes.length * 2 : 0) + (s.bookmarkIds ? s.bookmarkIds.length * 20 : 0) }
...
function totalUndoBytes() { if (_undoBytesDirty) _recalcUndoBytes(); return _totalUndoBytes }
...
export function restoreSnapshot(gid, snap) {
  const store = useAppStore()
  const sg = store.groupMap[gid]
  if (!sg) return
...
export function performUndo(gid) {
  const store = useAppStore()
  const undo = useUndoStore()
...
export function performRedo(gid) {
  const store = useAppStore()
  const undo = useUndoStore()
  const stack = undo.stacks[gid]
